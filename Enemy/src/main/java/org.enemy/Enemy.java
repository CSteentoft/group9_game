...
public class Enemy extends UserEntity {
    //Entity
    ...
    //HurtBox
    private final int enemyWidth = 20; //20
    private final int enemyHeight = 32; //32
    ...
    public Enemy(float xPos, float yPos) {
        ...
        entity.setCollisionBox(new Rectangle(entity.getPosition().x , entity.getPosition().y, enemyWidth, enemyHeight));
    ...
    public void updateAnimation(float dt){
        ...
        Gdx.app.log("Enemy", "Current animation index: " + animations.indexOf(currentAnimation));
    }
    ...
    public void moveHorizontal(float horizontalInput, float dt) {
        ...
        velocityX += horizontalInput * kEnemyInputAccel * dt;

        // Clamp speed to prevent exceeding max speed
        if (Math.abs(velocityX) > kEnemyMaxSpeed) {
            velocityX = Math.signum(velocityX) * kEnemyMaxSpeed;
        }
